...
【简写命令】（推荐，直接指定网盘）
  ...
  /天翼 [数量] <资源名>     搜索天翼云盘
  /移动 [数量] <资源名>     搜索移动云盘
  /pikpak [数量] <资源名>   搜索PikPak
  /磁力 [数量] <资源名>     搜索磁力链接

【通用命令】
  /搜索 [网盘类型] [数量] <资源名>
  示例: /搜索 百度 10 流浪地球  → 百度网盘，最多10条
  示例: /搜索 阿里 夸克 5 流浪地球 → 阿里+夸克，每类最多5条
  示例: /搜索 流浪地球          → 搜索全部网盘"""
...
class PanSouPlugin(Star):
    ...
    async def search(self, event: AstrMessageEvent):
        ...
        async for result in self._perform_search(event, keyword, cloud_types, limit):
            yield result
    ...
    async def _shortcut_search(self, event: AstrMessageEvent, cloud_type: str, cmd_name: str):
        """
        处理简写命令，如 /百度 10 有兽焉
        不依赖 event.command_str，直接通过 cmd_name 解析 message_str
        """
        text = event.message_str.strip()

        # 去掉命令前缀：/cmd_name 或 cmd_name
        for prefix in [f"/{cmd_name}", cmd_name]:
            if text.startswith(prefix):
                text = text[len(prefix):].strip()
                break

        # 解析参数：第一个数字是条数，其余是关键词
        parts = text.split()
        limit = None
        keyword_parts = []
        ...
        if not keyword:
            yield event.plain_result(f"❌ 请输入要搜索的资源名称。\n用法: /{cmd_name} [数量] <资源名>")
            return

        async for result in self._perform_search(event, keyword, [cloud_type], limit):
            yield result

    @filter.command("百度")
    async def cmd_baidu(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "baidu", "百度"):
            yield result

    @filter.command("阿里")
    async def cmd_ali(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "aliyun", "阿里"):
            yield result

    @filter.command("夸克")
    async def cmd_quark(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "quark", "夸克"):
            yield result

    @filter.command("天翼")
    async def cmd_tianyi(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "tianyi", "天翼"):
            yield result

    @filter.command("uc")
    async def cmd_uc(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "uc", "uc"):
            yield result

    @filter.command("115")
    async def cmd_115(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "115", "115"):
            yield result

    @filter.command("pikpak")
    async def cmd_pikpak(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "pikpak", "pikpak"):
            yield result

    @filter.command("迅雷")
    async def cmd_xunlei(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "xunlei", "迅雷"):
            yield result

    @filter.command("123")
    async def cmd_123(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "123", "123"):
            yield result

    @filter.command("移动")
    async def cmd_mobile(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "mobile", "移动"):
            yield result

    @filter.command("磁力")
    async def cmd_magnet(self, event: AstrMessageEvent):
        async for result in self._shortcut_search(event, "magnet", "磁力"):
            yield result
    ...
    def _is_valid_date(self, val: str) -> bool:
        if not val or not isinstance(val, str):
            return False
        invalid_patterns = [r"0000", r"0001", r"1970-01-01", r"0000-00-00"]
